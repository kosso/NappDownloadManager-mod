...
-(void)dealloc
{
    NSLog(@"[INFO] DISPATCH RELEASE!!");
    
    //dispatch_release(queue);  // kosso
...
defaultPermittedNetworks:(NetworkTypes)permittedNetworks
{
    ...
    dispatch_sync(queue, ^{
        ...
        NSLog(@"downloadRequests count: %lu", (unsigned long)[[self downloadRequests] count]);
    });
    [self persistToStorage];
...
-(DownloadRequest*)getDownloadRequest:(NSString*)url
{
    ...
    dispatch_sync(queue, ^{        
        ...
        for (DownloadRequest* item in [self downloadRequests])
        {
            if ([item.url caseInsensitiveCompare:blockUrl] == NSOrderedSame)
            {
                //NSLog(@"[INFO] getDownloadRequest found: %@", blockUrl);
                request = item;
                break;
            }
        }
...
-(DownloadRequest*)getNextDownloadCandidate:(NetworkTypes)network
{
    ...
    dispatch_sync(queue, ^{        
        ...
        {
            // NSLog(@"[INFO] DR = %@ - %ld", [request url], (long)[request status]);
...
-(void)resetQueue
{
    // NSLog(@"[INFO] resetQueue resetQueue resetQueue resetQueue resetQueue");
...
{  
    dispatch_sync(queue, ^{
        // NSLog(@"[INFO] Persisting queue to file");
        ...
        // NSLog(@"[INFO] Finished writing queue to file");
        [coder release];
    });
}
...
{
    // NSLog(@"[INFO] DownloadQueue loadFromStorage");
    dispatch_sync(queue, ^{        
        NSArray* paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, 
                                                             NSUserDomainMask, YES); 
        NSString* documentsDirectory = [paths objectAtIndex:0];
