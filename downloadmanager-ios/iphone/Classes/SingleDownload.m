...
{           
    ...
    if ([fileMan fileExistsAtPath:[self.downloadRequest filePath]] == false) {
        ...
        if (result == YES) {
            NSLog(@"[INFO] SINGLE : file successfully created: %@", [self.downloadRequest filePath]);
            ...
            NSLog(@"[INFO] Failed to create file. %@", [error debugDescription]);
        }
    }
    
    UInt32 size = [[fileMan attributesOfItemAtPath:[self.downloadRequest filePath] error:nil] fileSize];
    NSLog(@"[INFO] File Size %u of %lu", (unsigned int)size, (unsigned long)[self.downloadRequest length]);
...
-(void)startRequest
{
    ...
    if ([self.downloadRequest availableLength] > 0)
    {
        ...
        NSLog(@"[INFO] Setting Range Request %@", range);
        [request setValue:range forHTTPHeaderField:@"Range"];
    }
    
    NSLog(@"[INFO] Download Thread sending request.");
...
- (NSURLRequest *)connection:(NSURLConnection *)connection
 	...
 	NSLog(@"[INFO] SINGLE  Connection received data");
    return request;
}
...
-(void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    {
        //NSLog(@"[INFO] BUGGER");
        NSLog(@"Cannot continue. Download cancelled %@ - %ld", [self.downloadRequest url], (long)self.downloadRequest.status);
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection{
    ...
    // NSLog(@"[INFO] ***** CONNECTION FINISHED");
    NSLog(@"[INFO] connection finished loading:  %@", [self.downloadRequest filePath]);
    
    // This does not support chunked transfer..
    // so.. ugly chunked hack
    if (([self.downloadRequest availableLength] == [self.downloadRequest length])) {
    //if (([self.downloadRequest availableLength] == [self.downloadRequest length]) || ([self.downloadRequest lastDownloadBitsPerSecond] > 0  && [self.downloadRequest availableLength] > 0)) {
        ...
        // NSLog(@"[INFO] Download finished loading %lu / %lu", (unsigned long)[self.downloadRequest availableLength], (unsigned long)[self.downloadRequest length]);
        CFRunLoopStop(CFRunLoopGetCurrent());
    }
    else 
    {
        [urlConnection cancel];
        NSLog(@"[INFO] Download invalid lengths  :  %lu / %lu", (unsigned long)[self.downloadRequest availableLength], (unsigned long)[self.downloadRequest length]);
        NSLog(@"[INFO] Download invalid filePath :  %@", [self.downloadRequest filePath]);
        NSLog(@"[INFO] Download invalid url      : %@", [self.downloadRequest url]);
        ...
        if([self validateUrl:[self.downloadRequest url]]){
            ...
            NSLog(@"[INFO] ***** INVALID DOWNLOAD URL : %@", [self.downloadRequest url]);
...
-(void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    if(code >= 400){
        ...
        NSLog(@"[INFO] Error: Download is invalid. Response code: %ld", (long)code);
        [urlConnection cancel];
        [self.downloadRequest setAvailableLength:0];
    ...
    if ([httpResponse respondsToSelector:@selector(allHeaderFields)])
    {
        ...
        {
            //NSLog(@"[INFO] Server allows accept ranges so append to the file.");
        }
        
        long contentLength = httpResponse.expectedContentLength;
        // TiLog(@"[INFO] Content length %ld", contentLength);
...
{    
    // NSLog(@"[INFO] connection failed: %@", [error localizedDescription]);
    CFRunLoopStop(CFRunLoopGetCurrent());
...
- (BOOL)addSkipBackupAttributeToItemAtPath:(NSString *) filePathString
{
    ...
    if(!success){
        NSLog(@"[INFO] Error excluding %@ from backup %@", [URL lastPathComponent], error);
    }
    return success;
}
