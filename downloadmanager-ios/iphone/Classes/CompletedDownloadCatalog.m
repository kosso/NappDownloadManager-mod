...
{
    __block DownloadInformation* info = nil;
    dispatch_sync(queue, ^{        
        info = [downloadInformationsByUrl objectForKey:[self MD5:url]];
    }); 
    if(info == nil){
    //    NSLog(@"[INFO]  OH NOOOOOOO!!!!*!*!&@&*^!*&@^*&!^*&^@");
    }
    return info;
}
...
-(void)resetStorage
{
    NSLog(@"[INFO] RESET THE FUGGIN COMPLETED DOWNLOAD CATALOG!");
}

-(void)persistToStorage
{
    dispatch_async(queue, ^{
        NSLog(@"[INFO] Persisting item to catalog file");
        ...
        [coder release];
        NSLog(@"[INFO] Finished writing item to catalog file");
...
{
    dispatch_async(queue, ^{        
        NSLog(@"[INFO] CompletedDownloadCatalog loadFromStorage");
        NSArray* paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, 
                                                             NSUserDomainMask, YES); 
        NSString* documentsDirectory = [paths objectAtIndex:0];
        NSString* filePath = [documentsDirectory stringByAppendingPathComponent:@"/DownloadItemCatalog.dat"];
        ...
        if (fileExists == YES)
        {
            ...
            NSLog(@"[INFO] THERE IS NO DownloadItemCatalog.dat !");
        }
    });    
